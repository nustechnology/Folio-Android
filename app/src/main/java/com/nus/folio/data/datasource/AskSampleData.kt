...
/**
 * Shared suggestion catalog and local mock answers used by debug/release [AskDataSource].
 */
internal object AskSampleData {
    ...
    fun suggestionsFor(sourceId: String): List<String> {
        val normalized = sourceId.trim().takeIf { it.isNotEmpty() } ?: return emptyList()
        return sourceSuggestions[normalized].orEmpty().take(MAX_SUGGESTIONS)
    }
    ...
    fun mockAnswer(
        ...
        val scoped = sourceId?.trim()?.takeIf { it.isNotEmpty() }?.let { singleSourceAnswers[it] }
