...
/**
 * Central Folio API path and URL builders.
 * [BASE_URL] is `https://folio.nustechnology.com` for debug and release.
 */
internal object FolioApiPaths {

    const val BASE_URL = BuildConfig.FOLIO_API_BASE_URL
    ...
    fun spaceAsk(spaceId: String, baseUrl: String = BASE_URL): String =
        url(baseUrl, "$SPACES/${encode(spaceId)}/ask")

    fun spaceConversations(
        spaceId: String,
        baseUrl: String = BASE_URL,
        query: String? = null,
    ): String = urlWithQuery(baseUrl, "$SPACES/${encode(spaceId)}/conversations", query)

    fun spaceConversation(
        spaceId: String,
        conversationId: String,
        baseUrl: String = BASE_URL,
    ): String = url(
        baseUrl,
        "$SPACES/${encode(spaceId)}/conversations/${encode(conversationId)}",
    )

    fun spaceAskSuggestions(
        spaceId: String,
        baseUrl: String = BASE_URL,
        query: String? = null,
    ): String = urlWithQuery(baseUrl, "$SPACES/${encode(spaceId)}/ask/suggestions", query)

    fun spaceAskMessageFeedback(
        spaceId: String,
        conversationId: String,
        messageId: String,
        baseUrl: String = BASE_URL,
        ...
        baseUrl,
        "$SPACES/${encode(spaceId)}/conversations/${encode(conversationId)}/messages/${encode(messageId)}/feedback",
    )

    fun spaceNotebook(spaceId: String, baseUrl: String = BASE_URL): String =
        url(baseUrl, "$SPACES/${encode(spaceId)}/notebook")
